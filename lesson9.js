const menu = {
    rolls: [
        { name: 'California', price: 430, weight: '560g' },
        { name: 'Сalifornia with salmon', price: 580, weight: '600g' },
        { name: 'Philadelphia', price: 500, weight: '420g' },
        { name: 'Philadelphia with avocado', price: 550, weight: '490g' },
    ],
    drinks: [
        { name: 'Cola', price: 34, weight: '500ml' },
        { name: 'Fanta', price: 34, weight: '500ml' },
        { name: 'Sprite', price: 30, weight: '500ml' },
    ],
};
...
function addToOrder(itemIndex, quantity) {
    const item = itemIndex <= menu.rolls.length
...
function finalizeOrder() {
    ...
    order.items.forEach(item => {
        console.log(`${item.name} x${item.quantity} - ${item.price * item.quantity}UAH`);
    });
